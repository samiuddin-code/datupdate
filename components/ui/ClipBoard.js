    ...
    return (
        <>
            <div className="font-bold mb-3">
                {/* <div className={"inline-block relative top-2 mr-2 color-gray" + (animated ? "animate-bounce" : "")}>
                    <Image src={message} width="26" height="26" alt="Mail Icon Filled" />
                </div>  */}
                <span ref={copyTxt}>{email}</span>
            </div>
            <span className={(copied ? "text-green-900" : (animated ? "text-gray-900 opacity-80" : "text-gray-900 opacity-80")) + " cursor-pointer transition-all duration-300 ease-linear"} onClick={handleCopyTxt}>
                {
