...
const InputBox = ({labelText,children,classList,resize,textareaField,name,changeHandler,type,error,rounded,...rest}) => {
    ...
    return (
        <>
        {textareaField ? 
        <div className={(rounded ? "" : styles.inputboxHeight) + classList +' relative'}>
            ...
            <textarea onFocus={moveLabel} name={name} onBlur={moveLabel} onChange={changeHandler} id={name} className={(rounded ? `h-11 px-4 border rounded` : "border-b pt-7 ") + " outline-none transition-all duration-300 custom-transition  border-gray-900 border-opacity-20 h-full w-full focus:border-green-900"} {...rest} ></textarea>
            {
                errorMsg != '' &&
