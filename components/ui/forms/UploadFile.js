...
const UploadFile = ({name,error,formData,text,...rest}) => {
    ...
    return (
        <>
            <label htmlFor={name} className="w-full flex flex-row justify-center items-center h-16 cursor-pointer border rounded-md text-left text-sm py-2 pl-4 pr-10 capitalize leading-4 relative">                    
                <span className="relative z-10">{text}</span>
                <span className=" ml-4 text-xl leading-4 transition-all duration-300 custom-transition z-10">
                    <Upload/>
                </span>
            </label>
            {
