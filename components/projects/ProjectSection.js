          ...
          <SectionHeader span="Latest Design" heading={((projects.data.category) ?  projects.data.category.title : "Featured")+ " Projects"} />
            <div className="grid grid-cols-1 md:grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                projectData.map((item) => (
                    <Card key={item.title} heading={item.title} imageHover={true} fixedHeight={true} buttonData={{enabled:true,link:`/projects/${item.ProjectCategory.slug}/${item.slug}`,text:"See more",icon:true}} description={text_portion(item.highlight,0,200)} interactive={true}>
                    <ExternalImage src={item.altImage} layout="responsive" alt={item.title} width={352} height={260} objectFit="cover" className="filter blur-none group-hover:blur-sm" />
                    </Card>
                ))
                }
            </div>
