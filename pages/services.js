  ...
  return (
    <>
      <Layout apiData={response}>
        ...
        {/* Service Section */}
        <section className={(preloader ? "opacity-70 pointer-events-none" : "opacity-100 pointer-events-auto") + " py-12 lg:py-16 bg-white"}>
            ...
            <div className="grid grid-cols-1 md:grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {
                serviceData.map((item) => (
                  <ServiceCard key={item.title} heading={item.title} hover={true} fixedHeight={true} buttonData={{enabled:true,link:`/services/${item.slug}`,text:"Read more",icon:true}} description={text_portion(item.description,0,200)} interactive={true}>
              ...
              
              {
                ...
                <div className="pt-16 lg:pt-20 flex interactive-element top z-100">
